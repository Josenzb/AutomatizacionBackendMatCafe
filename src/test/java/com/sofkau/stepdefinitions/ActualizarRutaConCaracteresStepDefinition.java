...
public class ActualizarRutaConCaracteresStepDefinition extends ApiSetUp {
    public static Logger LOGGER = Logger.getLogger(ActualizarRutaConCaracteresStepDefinition.class);
    private final Ruta ruta = new Ruta();

    @Given("el administrador esta en el servicio de actualizar rutas")
    public void elAdministradorEstaEnElServicioDeActualizarRutas() {
        try {
    ...
    @When("envia  datos para actualizar id {string}, titulo {string}, descripcion {string}, duracion {string}, Id admin {string}")
    public void enviaDatosParaActualizarIdTituloDescripcionDuracionIdAdmin(String id, String title, String description, String duration, String adminId) {
        try {
            ruta.setTitle(title);
    ...
    @Then("no se actualizara la ruta junto con un status {int}")
    public void noSeActualizaraLaRutaJuntoConUnStatus(Integer code) {
        try {
            actor.should(
